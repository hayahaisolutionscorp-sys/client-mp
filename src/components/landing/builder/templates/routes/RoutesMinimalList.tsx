          ...
          {rest.length > 0 && (
            <div className="flex flex-col gap-4 w-full lg:w-[420px] lg:self-start">
              {rest.map((route, i) => (
                <div
                  key={route.id || i}
                  className="rounded-3xl p-4 flex items-center gap-5 border border-black/5"
                ...
                >
                  ...
                  <div className="flex-1">
                    <h4 className="text-base font-bold text-slate-800 line-clamp-1" style={{ color: theme.text }}>
                      {route.route}
                    </h4>
              ...
              {routes.length > visibleCount && (
                <button
                  ...
                  className="w-full py-4 text-sm font-black uppercase tracking-widest hover:opacity-80 rounded-2xl transition-all border border-dashed border-slate-200"
                  style={{ color: theme.primary, backgroundColor: theme.surface }}
                >
                  <span style={{ color: theme.primary }}>View All {routes.length - items.length} More Routes</span>
                </button>
              )}
            </div>
          )}
