...
export default function RegisterPasswordPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      // Reconstruct full RegisterForm from sessionStorage data + new password
      // Destructure out `ts` (TTL timestamp) so it doesn't leak into the API payload
  ...
  return (
    <main className="grid min-h-screen md:grid-cols-2">
      <div className="flex items-center justify-center p-6 lg:p-8">
        <button onClick={() => router.push('/register/email')}
        ...
        <div className="w-full max-w-md space-y-6">
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <div className="flex space-x-4">
              <Button type="button" variant="outline" className="w-full" onClick={() => router.push('/register/email')}>
                Back
              </Button>
              <Button type="submit" className="w-full text-white hover:scale-[1.02] active:scale-[0.98] transition-transform"
                style={{ backgroundColor: primaryColor }} disabled={loading}>
