...
import { ArrowLeft, UserIcon, Check, UserPlusIcon } from "lucide-react"
...
export default function RegisterPage() {
  ...
  useEffect(() => {
    clearSession();
    // Clear any stale register step data
  ...
  const [formData, setFormData] = useState<Omit<RegisterForm, 'email' | 'password' | 'confirm' | 'agreement' | 'emailConsent'>>({
    firstName: "",
    middleName: "",
    lastName: "",
    suffixName: "",
  ...
  const handleNext = (e: React.FormEvent) => {
    ...
    // Store personal info with timestamp — email/password are NOT here yet
  ...
  return (
    <main className="grid min-h-screen md:grid-cols-2">
      <div className="flex items-center justify-center p-6 lg:p-8">
        <Link href="/" className="absolute top-4 left-4 p-2 text-gray-600 hover:text-gray-900 transition-colors" aria-label="Back to homepage">
